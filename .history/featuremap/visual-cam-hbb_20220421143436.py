...
cfg = Config.fromfile("/home/zlx/mmdetection/work_dirs/GF-RarePlanes/faster_r50_on_newset/config.py")
model = build_detector(cfg.model, test_cfg=cfg.get('test_cfg')).cuda()

checkpoint = load_checkpoint(model, "/home/zlx/mmdetection/work_dirs/GF-RarePlanes/faster_r50_on_newset/epoch_24.pth", map_location='cuda')
...
def get_bboxes_tmp2(rpn_outs,
    ...
    for img_id in range(len(img_metas)):
        cls_score_list = [
            cls_scores[i][img_id] for i in range(num_levels) #保留了梯度
        ]
        bbox_pred_list = [
...
#         self.handlers.append(module.register_backward_hook(self._get_grads_hook))
    ...
    def __call__(self, inputs, index=0):
        ...
        x = self.net.extract_feat(inputs['img'][0].cuda())
