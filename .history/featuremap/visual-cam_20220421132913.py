...
model = build_detector(cfg.model, test_cfg=cfg.get('test_cfg'),device='cuda' )
...
def get_bboxes_tmp2(rpn_outs,
    ...
    for img_id in range(len(img_metas)):
        ...
        proposals = proposals.to(device='cuda:0')
        result_list.append(proposals)
    return result_list
...
#         self.handlers.append(module.register_backward_hook(self._get_grads_hook))
    ...
    def __call__(self, inputs, index=0):
        ...
        x = self.net.extract_feat(inputs['img'][0]).to('cuda:0')
        ...
        gradient = self.gradient.cuda().data.numpy().squeeze()
        
        weight = np.mean(gradient, axis=(1, 2))  # [C]

        # feature = self.feature[proposal_idx].cpu().data.numpy()  # [C,H,W]
        feature = self.feature.cuda().data.numpy().squeeze()
