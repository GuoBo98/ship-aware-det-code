...
然后回归loss直接用full-rois的bbox-pred
    分类loss计算时，cls-score是(top、middle、full)/3 + down的加权分数，这是为了让网络更注意尾部的结果。
    对应的代码在line-116左右
'''
import torch
...
class RoITransRoIHead_PCB_clsscore_weight(OBBBaseRoIHead):
    ...
    def _bbox_forward_train(self, stage, x, sampling_results, gt_bboxes,
        ...
        if self.current_stage == 0:
            ...
            obb_rois_top,obb_rois_middle,obb_rois_down = self.split_obb_rois(rois)
            bbox_results = self._bbox_forward(stage, x, rois)
            ...
            bbox_targets = self.bbox_head[stage].get_targets(
                sampling_results, gt_bboxes, gt_labels, rcnn_train_cfg)
            bbox_results['cls_score'] = bbox_results_top['cls_score'] + bbox_results['cls_score'] + bbox_results_down['cls_score'] 
            loss_bbox = self.bbox_head[stage].loss(bbox_results['cls_score'],
                                                bbox_results['bbox_pred'], rois,
                                                *bbox_targets)
            bbox_results.update(
