...
class OBBoxHead(nn.Module):
    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            ...
            pos_inds = (labels >= 0) & (labels < bg_class_ind)  #正样本所在的位置
    ...
    @force_fp32(
        apply_to=('cls_score', 'cls_score_top', 'cls_score_middle',
                  'cls_score_down', 'bbox_pred'))
    def loss_stage2(self,
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                losses['acc'] = accuracy(cls_score, labels)
                #top,down loss*2
                losses['loss_cls_top'] = self.loss_cls(
                    cls_score_top,
                    labels,
                    label_weights,
                    avg_factor=avg_factor,
                    reduction_override=reduction_override) * 2
                losses['acc_top'] = accuracy(cls_score_top, labels) 

                losses['loss_cls_middle'] = self.loss_cls(
                    cls_score_middle,
                    labels,
                    label_weights,
                    avg_factor=avg_factor,
                    reduction_override=reduction_override) 
                losses['acc_middle'] = accuracy(cls_score_middle, labels)

                losses['loss_cls_down'] = self.loss_cls(
                    cls_score_down,
                    labels,
                    label_weights,
                    avg_factor=avg_factor,
                    reduction_override=reduction_override) * 2
                losses['acc_down'] = accuracy(cls_score_down, labels)
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_bbox'] = bbox_pred.sum() * 0
        #将多个loss加权成一个loss
        new_losses['loss_cls'] = (losses['loss_cls_top']  + losses['loss_cls_down'] + losses['loss_cls_middle'] + losses['loss_cls'])/4
        new_losses['loss_bbox'] = losses['loss_bbox']
        return losses
    ...
    def get_bboxes(self,
        ...
        if rescale:
            ...
            bboxes = bboxes.view(
                bboxes.size(0), -1, get_bbox_dim(self.end_bbox_type))
    ...
    def regress_by_class(self, rois, label, bbox_pred, img_meta):
        ...
        """
        bbox_dim = get_bbox_dim(self.start_bbox_type)
        assert rois.size(1) == bbox_dim or \
                rois.size(1) == bbox_dim+1, repr(rois.shape)

        if not self.reg_class_agnostic:
            label = label * self.reg_dim
            inds = torch.stack([label + i for i in range(self.reg_dim)], 1)
            bbox_pred = torch.gather(bbox_pred, 1, inds)
        assert bbox_pred.size(1) == self.reg_dim

        if rois.size(1) == bbox_dim:
