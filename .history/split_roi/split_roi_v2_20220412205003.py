...
def split_roisv2(obb_rois):
    ...
    obb_rois_top_sort , indices = torch.sort(obb_rois_top,0)

    obb_rois_middle = torch.cat([new_idx,obb_rois_middle],1)
    obb_rois_middle_sort , indices = torch.sort(obb_rois_middle,0)

    obb_rois_down = torch.cat([new_idx,obb_rois_down],1)
    obb_rois_down_sort , indices = torch.sort(obb_rois_down,0)
