...
class OBBConvFCBBoxHead(nn.Module):
    def __init__(self):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.fc_cls = nn.Linear(in_features=1024,out_features=51,bias = True)
        self.fc_reg = nn.Linear(in_features=1024,out_features=5,bias = True)
        
        super().__init__()
    
    def forward(self,x:torch.Tensor):
        x = x.flatten(1)
        x = self.relu(self.fc_bn_1(self.shared_fc1(x)))
        x = self.relu(self.fc_bn_1(self.shared_fc1(x)))
...
'''BN'''
# self.norm1 = nn.BatchNorm2d(64) # C
# input = torch.randn(2,64,512,512)
# output = self.norm(input)

# '''LN'''
# input = torch.randn(2,64,512,512)
# self.norm2 = nn.LayerNorm(input.shape[1:]) '''LN不需要输入N这个维度'''
