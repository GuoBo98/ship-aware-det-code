...
def parse_results(config_file, resultfile, dstpath, type,epoch):
    ...
    if 'obb_results_dict' in vars():
        if not os.path.exists(os.path.join(dstpath, 'Task1_results-{}'.format(epoch))):
            os.makedirs(os.path.join(dstpath, 'Task1_results-{}'.format(epoch)))

        for cls in obb_results_dict:
            with open(os.path.join(dstpath, 'Task1_results-{}'.format(epoch), cls + '.txt'), 'w') as obb_f_out:
        ...
        '''occur some problems when doing nms ! '''
...
if __name__ == '__main__':
    epoch = 36
    config_file = "/home/guobo/AerialDetection/configs/ShipDet/v202-faster_rcnn_RoITrans_r50_fpn_1x_pretrained.py"
    config_name = os.path.splitext(os.path.basename(config_file))[0]
    pkl_file = os.path.join('/home/guobo/AerialDetection/work_dirs', config_name, 'results-{}.pkl'.format(epoch))
    output_path = os.path.join('/home/guobo/AerialDetection/work_dirs', config_name)
    type = 'OBB'
    parse_results(config_file, pkl_file, output_path, type,epoch)
