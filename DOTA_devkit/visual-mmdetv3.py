...
def main():
    ...
    device = 'cuda:0'
    save_path = '/data2/guobo/01_SHIPRSDET/mmdet-vis-test/'
    # build the model from a config file and a checkpoint file
    model = init_detector(config, checkpoint, device=device)
    # test a single image
    image_path = '/data2/guobo/01_SHIPRSDET/dota-format/split_ss_1024/val/images/'
    image_list = os.listdir(image_path)
    for i in trange(len(image_list)):
        img = image_list[i]
        image = cv2.imread(os.path.join(image_path, img))
        height, width, channels = image.shape
        result, x_fpn = inference_detector(model, image)
        ...
        cam_all = (cam_5 + cam_4 + cam_3 + cam_2) / 4
        ...
        heat_maps = np.hstack(
            (heatmap_image_2, heatmap_image_3, heatmap_image_4,
             heatmap_image_5, heatmap_image_all))
        results = np.hstack(
            (result_2, result_3, result_4, result_5, result_all))

        cv2.imwrite(
            os.path.join(save_path, img[0:-4] + '_heatmap.jpg'), heat_maps)
        cv2.imwrite(
            os.path.join(save_path, img[0:-4] + '_results.jpg'), results)
