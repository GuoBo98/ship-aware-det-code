...
img_path = "/data2/guobo/01_SHIPRSDET/ShipDetv2/Voc/train_set/images/"
out_path = "/data2/guobo/01_SHIPRSDET/ShipDetv2/train_crop_test0525/"
json_set = ["/data2/guobo/01_SHIPRSDET/ShipDetv2/Coco/annotations/shipRS_trainset.json"]
...
for j in range(len(json_set)):
    ...
    for i in tqdm.trange(num_bridges):
        ...
        long_edge = max(bridge.shape[0],bridge.shape[1])
        short_edge = min(bridge.shape[0],bridge.shape[1])
        if (long_edge*short_edge) > 6000 and (long_edge/short_edge) > 2.5 :
