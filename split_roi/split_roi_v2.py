...
def split_roisv2(obb_rois):
    ...
    obb_rois_top_sort = obb_rois_top[obb_rois_top[:,0].sort()[1]][:,1:]

    obb_rois_middle = torch.cat([new_idx,obb_rois_middle],1)
    obb_rois_middle_sort = obb_rois_middle[obb_rois_middle[:,0].sort()[1]][:,1:]

    obb_rois_down = torch.cat([new_idx,obb_rois_down],1)
    obb_rois_down_sort = obb_rois_down[obb_rois_down[:,0].sort()[1]][:,1:]
    
    
    return obb_rois_top_sort,obb_rois_middle_sort,obb_rois_down_sort
...
if __name__ == '__main__':
    ...
    a = time.time()
    obb_rois_top,obb_rois_middle,obb_rois_down = split_roisv2(obb_rois)
    b = time.time()
    print(b-a)
