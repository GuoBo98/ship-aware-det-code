...
class InstanceSwitch2:
    ...
    def __init__(self, data_root="/data2/guobo/01_SHIPRSDET/new-COCO-Format/", switch_prob=0.5,ann_path ="/data2/guobo/01_SHIPRSDET/new-COCO-Format/annotations/shipRS_trainset.json"):
        ...
        self.avg_class_num = math.floor(sum(list_class_num) / 50)
    ...
    def __call__(self, results):
        ...
        for ii,bridge_ in enumerate(bridges):
            ...
            if ori_cat_id in self.switch_class:  ##### 若该instance的类型是小类，则跳过
                continue
            if self.switch_num_of_more[self.more_class.index(ori_cat_id)] == 0: ####如果在大类且大类需要替换的目标数减为0，则跳过
                continue
            ...
            if self.get_distence(dst_pts[0], dst_pts[1]) > self.get_distence(dst_pts[1], dst_pts[2]):
                # Contract 1 pixel to avoid interpolation error.
                src_pts = np.array([[0 + 1, 100 - 1 - 1],
                                    [0 + 1, 0 + 1],
                                    [100 - 1 - 1, 0 + 1],
                                    [100 - 1 - 1, 100 - 1 - 1]], dtype="float32")
            else:
                src_pts = np.array([[0 + 1, 0 + 1],
                                    [100 - 1 - 1, 0 + 1],
                                    [100 - 1 - 1, 100 - 1 - 1],
                                    [0 + 1, 100 - 1 - 1]], dtype="float32")
            ...
            # # cv2.imwrite(os.path.join(switch_vis_path,filename[0:-4] + '-switch_target' + '.png'),switch_target)
            # # print(target_id)
            # # print(switch_plane[0])
            # # target_id = switch_plane[0]
            # # switch_target = switch_plane[1]
        ...
        cv2.imwrite(os.path.join(switch_vis_path,filename[0:-4] + '-switched' + '.png'),img_cv)
        return results