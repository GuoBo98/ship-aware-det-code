...
class OBBGenericRoIExtractorSmall(OBBBaseRoIExtractor):
    ...
    def forward(self, feats, rois):
        ...
        idx_small = torch.where(rois[:,3] * rois[:,4] <= 10000)[0]
        idx_big = torch.where(rois[:,3] * rois[:,4] > 10000)[0]

        rois_big = rois[idx_big]
        rois_small = rois[idx_small]
        ...
        for i in range(num_levels):
            ...
            if self.aggregation == 'sum':
                # and sum them all
                roi_feats_big = roi_feats_big + roi_feats_t_big
            else:
        ...
        for i in range(2):
            ...
            if self.aggregation == 'sum':
                # and sum them all
                roi_feats_small = roi_feats_small + roi_feats_t_small
            else:
                # and concat them along channel dimension
        ...
        roi_feats[idx_small] = roi_feats_small
        roi_feats[idx_big] = roi_feats_big
