'''这个head新加了一个特征提取器bbox_roi_extractor,用于提取C2C3比较低等级的语义信息'''
'''修改了line50的这个判断'''
import torch
...
class RoITransRoIHeadSingleExt(OBBBaseRoIHead):
    ...
    def __init__(self,
        ...
        super(RoITransRoIHeadSingleExt, self).__init__(
    ...
    def init_bbox_head(self, bbox_roi_extractor, bbox_head):
        ...
        assert len(bbox_roi_extractor) == len(bbox_head) +1  == self.num_stages + 1
        ...
        '''再加一个bbox_roi_extractor'''
        self.bbox_roi_extractor.append(build_roi_extractor(bbox_roi_extractor[-1]))
    ...
    def _bbox_forward(self, stage, x, rois):
        if stage == 0:
            ...
            cls_score, bbox_pred = bbox_head(bbox_feats,bbox_feats,stage)
            ...
            bbox_roi_extractor = self.bbox_roi_extractor[stage]
            bbox_roi_extractor_single_fpn = self.bbox_roi_extractor[stage+1]
            bbox_head = self.bbox_head[stage]
            bbox_feats = bbox_roi_extractor(x[:bbox_roi_extractor.num_inputs], rois)
            bbox_feats_cls = bbox_roi_extractor_single_fpn(x[:bbox_roi_extractor_single_fpn.num_inputs], rois)
            cls_score, bbox_pred = bbox_head(bbox_feats,bbox_feats_cls,stage)
