'''
这个head是将obb-rois进行分割，然后将top、middle、down、full-rois这四部分分别foword得到cls-score和bbox-pred，
然后回归loss直接用full-rois的bbox-pred
    分类loss计算时，cls-score是top、middle、full + down的加权分数，这是为了让网络更注意头部和尾部的结果。
    对应的代码在line-116左右
'''
...
class RoITransRoIHead_PCB_clsscore_weight(OBBBaseRoIHead):
    ...
    def __init__(self,
        ...
        super(RoITransRoIHead_PCB_clsscore_weight, self).__init__(
    ...
    def _bbox_forward_train(self, stage, x, sampling_results, gt_bboxes,
        ...
        if self.current_stage == 0:
            ...
            obb_rois_top, obb_rois_middle, obb_rois_down = self.split_obb_rois(
                rois)
            bbox_results = self._bbox_forward(stage, x, rois)
            bbox_results_top = self._bbox_forward(stage, x, obb_rois_top)
            bbox_results_middle = self._bbox_forward(stage, x, obb_rois_middle)
            bbox_results_down = self._bbox_forward(stage, x, obb_rois_down)
            bbox_targets = self.bbox_head[stage].get_targets(
                sampling_results, gt_bboxes, gt_labels, rcnn_train_cfg)
            bbox_results[
                'cls_score'] = bbox_results_top['cls_score'] + bbox_results[
                    'cls_score'] + bbox_results_down['cls_score']
            loss_bbox = self.bbox_head[stage].loss(bbox_results['cls_score'],
                                                   bbox_results['bbox_pred'],
                                                   rois, *bbox_targets)
            bbox_results.update(
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        '''这个test是经过修改的，采用和训练一样的模式，切割后对各个部分，分别求分数计算而来的。'''
        ...
        for i in range(self.num_stages):
            if i == 0:
                ...
                obb_rois_top, obb_rois_middle, obb_rois_down = self.split_obb_rois(
                    rois)
                bbox_results = self._bbox_forward(i, x, rois)
                bbox_results_top = self._bbox_forward(i, x, obb_rois_top)
                bbox_results_middle = self._bbox_forward(i, x, obb_rois_middle)
                bbox_results_down = self._bbox_forward(i, x, obb_rois_down)
                bbox_results['cls_score'] = bbox_results_top[
                    'cls_score'] + bbox_results[
                        'cls_score'] + bbox_results_down['cls_score']
                ms_scores.append(bbox_results['cls_score'])
