...
class RoITransRoIHead_PCB_cat_fc(OBBBaseRoIHead):
    ...
    def __init__(self,
        ...
        super(RoITransRoIHead_PCB_cat_fc, self).__init__(
    ...
    def split_obb_rois(self, obb_rois):
        ...
        weight_top, weight_middle, weight_down = torch.tensor([0.2, 0.6, 0.2],dtype=torch.float32,device='cuda')
        ...
        for i in range(len(obb_rois)):
            ...
            tensor_middle = torch.tensor(
                [roi_idx, x, y, w * weight_middle, h, theta],dtype=torch.float32).cuda()
            ...
            if theta < (pi / 2) and theta > 0:
                ...
                y_down = y + ((weight_down + weight_middle) * w / 2) * abs(
                    math.sin(theta))
                w_down = w * weight_down
            tensor_top = torch.tensor([roi_idx, x_top, y_top, w_top, h, theta],dtype=torch.float32,device='cuda').cuda()
            tensor_down = torch.tensor(
                [roi_idx, x_down, y_down, w_down, h, theta],dtype=torch.float32,device='cuda').cuda()
    ...
    def _bbox_forward_orignal(self, stage, x, rois):
        bbox_roi_extractor = self.bbox_roi_extractor[stage]
        bbox_head = self.bbox_head[stage]
    ...
    def _bbox_forward(self, stage, x, rois,rois_top, rois_middle,
                             rois_down):
        if stage == 0:
            bbox_roi_extractor = self.bbox_roi_extractor[stage]
            bbox_head = self.bbox_head[stage]
    ...
    def _bbox_forward_train(self, stage, x, sampling_results, gt_bboxes,
        ...
        if self.current_stage == 0:
            bbox_results = self._bbox_forward(stage, x, rois,rois,rois,rois)
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        for i in range(self.num_stages):
            if i == 0:
                bbox_results = self._bbox_forward(i, x, rois,rois,rois,rois)
                ...
                obb_rois_top, obb_rois_middle, obb_rois_down = self.split_obb_rois(
                    rois)
                timeend = time.time()
                print('split_time:', timeend - timestart)
